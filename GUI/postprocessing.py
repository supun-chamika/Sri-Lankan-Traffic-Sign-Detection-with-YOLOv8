...
def draw_box(img, result, class_list, box_thickness, label_text_size) :
    ...
    for run_output in sum_output :
        ...
        cv2.rectangle(out_image, first_half_box, second_half_box, box_color, thickness=box_thickness)
        # Create text
        ...
        labelSize, baseLine = cv2.getTextSize(text_print, cv2.FONT_HERSHEY_SIMPLEX, label_text_size, box_thickness) 
        ...
        cv2.putText(out_image, text_print ,text_location
                    , cv2.FONT_HERSHEY_SIMPLEX , label_text_size
                    , text_color, box_thickness ,cv2.LINE_AA)
    return out_image